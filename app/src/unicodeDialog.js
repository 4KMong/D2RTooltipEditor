...
function updatePreview() {
  ...
  previewGlyphDefault.style.fontFamily = `"${BUILTIN_FONT_CSS_FAMILY}", sans-serif`;
...
function selectCode(code, scroll = false) {
  ...
  gridItems?.querySelectorAll('.glyph-cell').forEach(cell => {
    const selected = cell.dataset.code === clean;
...
function makeCell(code, favorites) {
  ...
  const selected = code === state.selectedCode;
  const favorite = favorites.has(code);
  const isDefault = code === state.defaultCode;
  const supported = state.supportedSet.has(code);
  cell.className = `glyph-cell${selected ? ' selected' : ''}${!supported ? ' glyph-not-supported' : ''}`;
  cell.dataset.code = code;
  cell.setAttribute('role', 'option');
  cell.setAttribute('aria-selected', selected ? 'true' : 'false');
  if (!supported) cell.setAttribute('data-tip', '현재 출력 폰트에서는 이 글리프를 지원하지 않습니다.');
  cell.innerHTML = `
    <button type="button" class="glyph-favorite-toggle${favorite ? ' active' : ''}" aria-label="즐겨찾기 ${favorite ? '해제' : '등록'}" data-glyph-favorite>${favorite ? '★' : '☆'}</button>
  ...
  cell.querySelector('[data-glyph-favorite]').addEventListener('click', event => {
    event.preventDefault();
...
async function refreshFontGlyphs() {
  ...
  try {
    ...
    const [outputResult, vanillaResult] = await Promise.allSettled([
      loadFontCmapCodes(entry),
      loadFontCmapCodes({ source: 'builtin' }),
    ]);
    if (sequence !== loadSequence || !unicodeWindow) return;
    if (vanillaResult.status === 'fulfilled') state.vanillaSupportedSet = new Set(sortCodes(vanillaResult.value));
...
function moveSelectionLinear(delta) {
  ...
  selectCode(state.displayCodes[next], true);
}
...
function toggleFavorite(code) {
  ...
  const adding = !set.has(clean);
  if (adding) set.add(clean);
  else set.delete(clean);
  state.favoriteCodes = sortCodes([...set]);
  saveState();
  renderVirtualGrid();
  selectCode(clean, true);
  setStatusMessage(`${unicodeAddress(clean)} 즐겨찾기 ${adding ? '등록' : '해제'}`);
}
...
async function resetFavorites() {
  if (!await showConfirmModal('유니코드 즐겨찾기와 기본값을 초기 상태로 되돌리시겠습니까?', { title: '즐겨찾기 초기화' })) return;
  const settings = resetGlyphSettings();
  state.favoriteCodes = sortCodes(settings.favoriteCodes || settings.codes || []);
  state.defaultCode = settings.defaultCode;
  state.selectedCode = settings.defaultCode;
  rebuildDisplayCodes();
  renderVirtualGrid();
  selectCode(state.selectedCode, true);
  updateGlyphSummary();
  setStatusMessage('유니코드 즐겨찾기 초기화');
}
...
function showGlyphContextMenu(x, y) {
  ...
  const menuCode = state.selectedCode;
  const isFavorite = favoriteSet().has(menuCode);
  const items = [
    [state.selectedCode === state.defaultCode && state.defaultCode !== '2060' ? '기본값 해제' : '기본값으로 지정', setOrClearDefault],
    [isFavorite ? '즐겨찾기 해제' : '즐겨찾기 등록', () => toggleFavorite(menuCode)],
  ];
...
function createContent() {
  ...
  root.innerHTML = `
    ...
    <div class="unicode-main">
      ...
      <aside class="unicode-preview">
        <div class="unicode-preview-panel unicode-preview-default">
          <div class="unicode-preview-label">기본 폰트 <button type="button" class="unicode-help-icon" aria-label="기본 폰트 설명" data-tip="수정되지 않은 D2R 바닐라 Kodia 폰트입니다. 현재 출력 폰트의 커스텀 유니코드 글리프가 바닐라 게임 화면에서는 어떻게 보일지 비교하기 위한 참고용입니다.">?</button></div>
    ...
    <div class="unicode-actions">
      ...
      <button type="button" data-unicode-action="reset">즐겨찾기 초기화</button>
  ...
  root.querySelector('[data-unicode-action="reset"]').addEventListener('click', resetFavorites);
  ...
  grid.addEventListener('keydown', event => {
    if (event.key === 'Enter') { event.preventDefault(); inputSelected(false); }
    ...
    else if (event.key === 'Home') { event.preventDefault(); selectCode(state.displayCodes[0], true); }
    else if (event.key === 'End') { event.preventDefault(); selectCode(state.displayCodes[state.displayCodes.length - 1], true); }
  });
